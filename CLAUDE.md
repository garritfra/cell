...
cargo test -p cell-core                  # test only the core library
cargo test -p cell-tui                   # test only the TUI crate
cargo test -p cell-core -- col_label     # run a single test by name
cargo fmt --check                        # check formatting
cargo clippy                             # lint
cargo install --path crates/cell-tui     # install the `cell` binary
```
...
- **`cell-core`** — Pure data library with zero TUI dependencies. Contains the data model (`Sheet`, `Cell`, `CellValue`, `CellPos`), formula engine (tokenizer → parser → AST → evaluator), dependency graph with topological recalculation, and file I/O (CSV/TSV via the `csv` crate, native `.cell` format).
- **`cell-tui`** — Terminal UI built on `ratatui`/`crossterm`. Implements Vim modal editing (Normal, Insert, Visual, VisualBlock, Command modes), the main event loop, rendering, undo/redo, clipboard with formula-aware paste, and viewport scrolling.
...
### Formula engine pipeline (cell-core)
