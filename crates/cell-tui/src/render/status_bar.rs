...
impl<'a> Widget for StatusBar<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let file_info = match self.file_name {
            Some(name) => if self.dirty { format!(" {} [+]", name) } else { format!(" {}", name) },
            None => if self.dirty { " [No Name] [+]".into() } else { " [No Name]".into() },
        };
        let info_x = area.x + mode_str.len() as u16;
        buf.set_string(info_x, area.y, &file_info, style);

        let right = format!("{} rows x {} cols │ {}{} ",
            self.row_count, self.col_count,
            cell_core::model::col_index_to_label(self.cursor.1), self.cursor.0 + 1);
