...
fn parse_sort_command(args: &str) -> Action {
    ...
    if parts.is_empty() { return Action::Noop; }
...
mod tests {
    ...
    #[test]
    fn parse_force_quit() { assert_eq!(parse_command("q!"), Action::Quit { force: true }); }

    #[test]
    fn parse_write() { assert_eq!(parse_command("w"), Action::Save(None)); }

    #[test]
    fn parse_write_path() { assert_eq!(parse_command("w foo.csv"), Action::Save(Some(PathBuf::from("foo.csv")))); }

    #[test]
    fn parse_force_write() { assert_eq!(parse_command("w!"), Action::ForceSave(None)); }

    #[test]
    fn parse_edit() { assert_eq!(parse_command("e data.csv"), Action::Open(PathBuf::from("data.csv"))); }

    #[test]
    fn parse_sort_asc() { assert_eq!(parse_command("sort A asc"), Action::Sort { col: 0, ascending: true }); }

    #[test]
    fn parse_sort_desc() { assert_eq!(parse_command("sort B desc"), Action::Sort { col: 1, ascending: false }); }
    ...
    #[test]
    fn command_esc_cancels() { assert_eq!(handle_command_key(key(KeyCode::Esc), ""), CommandAction::Cancel); }

    #[test]
    fn command_enter_executes() { assert_eq!(handle_command_key(key(KeyCode::Enter), "w"), CommandAction::Execute("w".into())); }

    #[test]
    fn command_char_inserts() { assert_eq!(handle_command_key(key(KeyCode::Char('q')), ""), CommandAction::InsertChar('q')); }
}
